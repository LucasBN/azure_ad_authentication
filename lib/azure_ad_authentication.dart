...
class AzureAdAuthentication {
  ...
  AzureAdAuthentication._create({required MsalConfig config}) {
    _config = config;
  }
  ...
  static Future<AzureAdAuthentication> createPublicClientApplication(
    ...
    var res = AzureAdAuthentication._create(config: config);
  ...
  Future<UserAdModel?> acquireToken({
    ...
    try {
      print("here");
      final result =
          await _channel.invokeMapMethod<String, dynamic>('acquireToken', res);
      print("finished here");
